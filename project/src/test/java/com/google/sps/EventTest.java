...
public final class EventTest {
  ...
  private static final Set<User> NEW_ATTENDEES =
      new HashSet<User>(Arrays.asList(USER1, USER2, USER3));

  /* User creating event
   * Verify Builder class is created with correct required and optional fields */
  @Test
  public void testEventBuild() {
    Event event = new Event.Builder(EVENT_NAME, DESCRIPTION, LABELS, LOCATION, DATE, HOST)
  ...
  public void setEventFields() {
    ...
    Event.Builder changedEventBuilder =
        event.toBuilder().setAttendees(ATTENDEES).build().toBuilder();
